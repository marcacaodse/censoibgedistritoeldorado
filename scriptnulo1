document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderTables(tableData) {
        ...
        // Nova tabela do censo 2021
        renderCensusTable(tableData.censusTableData);
    }

    function renderCensusTable(data) {
    ...
    // Função de busca na tabela do censo
    ...
    [ubsFilter, vulnerabilityFilter, ageGroupFilter, districtFilter].forEach(filter => {
        filter.addEventListener("change", renderDashboard);
    });

    censusTableSearch.addEventListener("input", filterCensusTable);
