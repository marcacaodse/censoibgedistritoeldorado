document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderDashboard() {
        ...
        let currentEldoradoPopulation = 0;
        if (selectedFilters.        // Atualizar estatísticas
        let currentEldoradoPopulation = 0;
        if (selectedFilters.ubs.length === 0) {
            // Se nenhum filtro de UBS estiver selecionado, use a população original do Eldorado
            currentEldoradoPopulation = parseInt(data.eldoradoPopulation.replace(/\./g, ""));
        } else {
            // Caso contrário, calcule com base nas UBS filtradas
            currentEldoradoPopulation = tableData.ubsTableData.reduce((sum, ubs) => sum + ubs.population, 0);
        }
        document.getElementById("eldoradoPopulation").textContent = currentEldoradoPopulation.toLocaleString("pt-BR");
        document.getElementById("totalPopulation").textContent = data.totalPopulation;
        document.getElementById("totalUBS").textContent = tableData.ubsTableData.length.toString();
